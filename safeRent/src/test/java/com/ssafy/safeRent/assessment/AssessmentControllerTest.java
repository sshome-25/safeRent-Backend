...
public class AssessmentControllerTest {
    ...
    void setUp() {
        ...
        // 테스트용 요청 객체 설정
        assessmentRequest = AssessmentRequest.builder()
        		.build();
    ...
    void gradeAssessmentTest() throws Exception {
        // Given
        when(assessmentService.gradeAssessment(any(AssessmentRequest.class)))
    ...
    @Test
    @DisplayName("등기부 등록 API 테스트")
    @WithMockUser
    void saveRegisterTest() throws Exception {
        // Given
        doNothing().when(assessmentService).saveRegister(any(RegisterRequest.class));

        // When & Then
        mockMvc.perform(post("/api/assessments/register")
                .with(SecurityMockMvcRequestPostProcessors.user(mockUser))
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void saveContractTest() throws Exception {
        ...
        // When & Then
        mockMvc.perform(post("/api/assessments/contract")
                .with(SecurityMockMvcRequestPostProcessors.user(mockUser))
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void getRegisterAnalyzeTest() throws Exception {
        ...
        // When & Then
        mockMvc.perform(get("/api/assessments/register", registerId)
                .with(SecurityMockMvcRequestPostProcessors.user(mockUser))
    ...
    void getContractAnalyzeTest() throws Exception {
        ...
        // When & Then
        mockMvc.perform(get("/api/assessments/contract")
        	    .with(SecurityMockMvcRequestPostProcessors.user(mockUser))
